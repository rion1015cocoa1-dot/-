...
### 2.1 対応SNSプラットフォーム別の評価

| プラットフォーム | データ取得難易度 | 取得可能情報 | 推奨手法 | 実現性 |
|-----------------|----------------|-------------|---------|--------|
| **X (Twitter)** | 中 | プロフィール、直近ツイート、bio | X API v2 (Basic: $100/月) | **◎ 高い** |
| **Instagram** | 高 | プロフィール、bio、投稿キャプション | Instagram Basic Display API / スクレイピング | **△ 制約あり** |
| **LinkedIn** | 高 | 役職、会社、経歴 | 公開プロフィールのスクレイピング | **△ ToS注意** |
| **Facebook** | 非常に高 | 公開プロフィールのみ | Graph API (限定的) | **× 非推奨** |
| **note / Zenn** | 低 | 記事内容、プロフィール | 公開ページのスクレイピング | **◎ 高い** |
| **YouTube** | 低 | チャンネル説明、動画タイトル | YouTube Data API v3 (無料枠あり) | **◎ 高い** |
...
| プラットフォーム | スクレイピング | API利用 | 注意点 |
|-----------------|--------------|---------|--------|
| X | 規約上禁止 | **API利用を推奨** | Automated DM送信は別途制限あり |
| Instagram | 規約上禁止 | Basic Display API利用 | ビジネスアカウントのみ詳細取得可 |
| LinkedIn | 明確に禁止 | 公式APIは審査制 | **最もリスクが高い** |
| note | 明確な規定なし | APIなし | 公開情報のみ利用 |

### 10.2 対策

1. **公開情報のみ利用**: 非公開アカウントの情報は取得しない
2. **API優先**: 可能な限り公式APIを使用する
3. **キャッシュ管理**: 取得データは24時間で更新、ユーザー削除に対応
4. **利用規約の明示**: ユーザーに各プラットフォームの規約遵守を促す
5. **レート制限の遵守**: 各APIのレート制限を厳密に守る
6. **オプトアウト**: 生成されたDMの送信はユーザーが手動で行う（自動送信しない）
...
| **X (Twitter) 対応** | **◎** | API v2で安定的にデータ取得可能 |
| **Instagram 対応** | **△** | API制約があり限定的 |
| **LinkedIn 対応** | **△** | ToS上リスクあり、慎重な対応が必要 |
| **AI生成品質** | **◎** | GPT-4oで十分な品質のDM生成が可能 |
| **コスト** | **◎** | 月$110〜155で1,000件対応可能 |
| **法的リスク** | **○** | 公開情報+公式API利用で対応可能 |
| **市場ニーズ** | **◎** | 営業効率化ニーズは非常に高い |

**結論: 本ツールは技術的に十分実現可能。Phase 1 (MVP) であれば既存の技術スタックをほぼそのまま活用して短期間で構築可能。まずはX (Twitter) に絞ったMVPから開始し、反応を見ながら拡張していくアプローチを推奨。**
